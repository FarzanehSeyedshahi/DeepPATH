...
parser.add_argument("--background", type=int, default=90, help='Maximum background tolerated')
...
def tile_extractor(image, tile_size, pixel_size, magnification, background, output, format):
    ...
    lazy_t0 = img.get_image_dask_data("XYS")  
    ...
    print(f'Tiles Range X: {tiles_range_x}')
    print(f'Tiles Range Y: {tiles_range_y}')
    for i in range(0, tiles_range_x + 1):
        for j in range(0, tiles_range_y + 1):
            ...
            tile = img_array[i * scaled_tile:(i+1) * scaled_tile, j * scaled_tile:(j+1) * scaled_tile, :]
